...
*Setup instructions for a gitian build of TWINS using a Debian VM or physical system.*

* this process is not yet implemented ! *

Gitian is the deterministic build process that is used to build the TWINS
...
Clone the git repositories for twins and gitian and then checkout the twins version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the twins repository
...
To build TWINS (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the twins repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/twins/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/NewCapital/TWINS-Core.git
...
For example:
```bash
URL=https://github.com/crowning-/twins.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit twins=${COMMIT} --url twins=${URL} ../twins/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit twins=${COMMIT} --url twins=${URL} ../twins/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit twins=${COMMIT} --url twins=${URL} ../twins/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/twins-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/twins-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/twins-build.assert
```
...
[twins/gitian.sigs](https://github.com/NewCapital/TWINS-gitian.sigs/) repository, or if that's not possible to create a pull
request.
