...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.ztwinsTracker = unique_ptr<CzTWINSTracker>(new CzTWINSTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZTWINS_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZTWINS_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
